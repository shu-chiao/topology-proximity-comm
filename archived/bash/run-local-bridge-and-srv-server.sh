#!/usr/bin/env bash
# zenoh_bridge-as-srv-server-client.json5 + ros2 run demo_nodes_cpp add_two_ints_server (/add_two_ints).
...
CONFIG="${ZENOH_BRIDGE_CONFIG:-$ROOT/configs/zenoh_bridge-as-srv-server-client.json5}"
...
if [[ -n "${ZENOH_BRIDGE_ROUTER:-}" ]]; then
  ...
  echo "(add_two_ints_server script) zenoh-bridge ← $CONFIG  explicit ZENOH_BRIDGE_ROUTER=${ZENOH_BRIDGE_ROUTER}"
else
  echo "(add_two_ints_server script) zenoh-bridge ← $CONFIG  (Zenoh multicast scouting)  ROS_AUTOMATIC_DISCOVERY_RANGE=${ROS_AUTOMATIC_DISCOVERY_RANGE:-unset}"
fi
