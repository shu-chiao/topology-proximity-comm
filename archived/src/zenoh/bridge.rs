...
fn config_path_for_mode(mode: BridgeMode, manifest: &PathBuf) -> PathBuf {
    manifest.join("configs").join(mode.config_basename())
}
...
pub fn spawn(opts: &BridgeSpawnOptions) -> anyhow::Result<(Option<BridgeKillOnDrop>, BridgeMode)> {
    ...
    let cfg_path = config_path_for_mode(mode, &manifest);
